...
float WINAPI KeyDown()                          { return GetDXUTState().GetElapsedTime(); }
                                 ...
                                 HICON hIcon, HMENU hMenu, int x, int y )
{
    ...
    if( DXUTGetHWNDFocus() == NULL )
    {
        ...
        int nDefaultWidth = WINSIZEX;
        int nDefaultHeight = WINSIZEY;
...
bool WINAPI KeyPress( BYTE vKey )
{
    bool* bKeys = GetDXUTState().GetKeys();
