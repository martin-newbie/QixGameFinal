...
bool      WINAPI KeyPress( BYTE vKey ); // Pass a virtual-key code, ex. VK_F1, 'A', VK_RETURN, VK_LSHIFT, etc
bool      WINAPI KeyDown( BYTE vKey );  // Like DXUTIsKeyDown() but return true only if the key was just pressed
